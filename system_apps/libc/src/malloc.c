...
#define PAGE_ALIGN_UP(value)  math_ALIGN_UP(value, PAGE_SIZE)
...
{
    void *allocated_addr = sbrk(PAGE_SIZE);
    assert(allocated_addr != (void *)-1);
...
void *malloc_from_top(size_t victim_size)
{
    ...
    {
        bool rs = malloc_grow_heap(victim_size);
        if (!rs)
            return NULL;
    }

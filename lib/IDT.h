...
typedef struct
{
    ...
    uint16_t selector; //represent the segmenet in which the function located
    uint8_t zero; //it has 0 meaning but it must be 0
    uint8_t flags; //holds the flags of the intterupt (check out the IDT_FLAGS enum)
...
typedef struct 
{
    uint16_t limit; //spesifies the maximum size of the IDT (IDT_MAX_ENTRIES * Entry size)
    uint64_t base; //theb base address of the idt
...
typedef enum
{
    ...
    IDT_FLAG_GATE_INT16 =  0x5, //A 16-bit intterupt gate , for handling 16-bit mode intterupts 
    ...
    IDT_FLAG_RING3      =  0x60,//sets the privilege level to 3 meaming it can also be accesed by user mode
...
extern void idt_flush(uint64_t idt_ptr_address); //needs to be an ass implementaion
